...
rows_1 = parse_csv_block(csv_block_1)
rows_2 = parse_csv_block(csv_block_2)

all_rows = rows_1 + rows_2

print(f"Parsed {len(all_rows)} total rows from block 1 & 2.")
...
print(f"Valid rows with barcodes: {len(valid_new_rows)}")

# Deduplicate valid_new_rows on barcode (keep first)
seen_barcodes = set()
deduped_new_rows = []
for r in valid_new_rows:
    bc = r['barcode'].strip()
    if bc not in seen_barcodes:
        seen_barcodes.add(bc)
        deduped_new_rows.append(r)

print(f"Deduplicated new rows: {len(deduped_new_rows)}")
...
for row in deduped_new_rows:
    ...
    is_ing_empty = not ing or ing.strip().lower() in ['nan', 'none', '', 'pending capture']
